	...
	do
	{
		...
		else
		{
			...
			if (n >= sizeof(size_t))
			{
				...
				{
					if (debug)
						fprintf(stderr, "%lu : [%lu, %lu]\n", (unsigned long)goal, (unsigned long)minlen, (unsigned long)maxlen);
					return HES_BAD_LEN;
				}
			}
		}
...
#define DESERIALIZE_THING(thing) DESERIALIZE_THING_LEN(thing, sizeof(*thing))
	...
	while (size_left >= SIZEOF_PREAMBLE)
	{
		...
		if (size_left >= t1.ai_addrlen && t1.ai_addrlen < 1024 /* impose arbitrary big maximum */
				&& t1.ai_addrlen > 0)
		{
...
worker(int qfd, int afd)
{
	...
	for (;;)
	{
		...
		enum helper_exit_status r = read_pipe(qfd, (unsigned char *)&q, sizeof(q), sizeof(q));
		...
		case ns_t_a:
			...
			default:
				openswan_log("adns lookup: %s a/aaaa lookup error: %s", q.name_buf, gai_strerror(s));
				a.h_errno_val = s;
				a.result = -1;
				break;
			}
...
#ifdef DEBUG
		if (((q.debugging & IMPAIR_DELAY_ADNS_KEY_ANSWER) && q.type == ns_t_key) || ((q.debugging & IMPAIR_DELAY_ADNS_TXT_ANSWER) && q.type == ns_t_txt))
...
forward_query(struct worker_info *w)
{
	...
	{
		enum helper_exit_status r = write_pipe(w->qfd, (const unsigned char *)&q->aq);
...
query(void)
{
	...
	r = read_pipe(PLUTO_QFD, (unsigned char *)&q->aq, sizeof(q->aq), sizeof(q->aq));
...
{
	struct adns_answer a;
	enum helper_exit_status r = read_pipe(w->afd, (unsigned char *)&a, offsetof(struct adns_answer, ans), sizeof(a));
	...
	else
	{
		...
		enum helper_exit_status rs = write_pipe(PLUTO_AFD, (const unsigned char *)&a);
