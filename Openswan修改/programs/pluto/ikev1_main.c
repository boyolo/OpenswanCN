...
/* Initiate an Oakley Main Mode exchange.   启动 Oakley 主模式交换。
 * --> HDR;SA   --> HDR；SA
 * Note: this is not called from demux.c   注意：这不是从 demux.c 调用的
 */
stf_status
main_outI1(int whack_sock, struct connection *c, struct state *predecessor, so_serial_t *newstateno, lset_t policy, unsigned long try, enum crypto_importance importance, struct xfrm_user_sec_ctx_ike *uctx)
{
	...
	if (HAS_IPSEC_POLICY(policy))
		add_pending(dup_any(whack_sock), st, c, policy, 1, predecessor == NULL ? SOS_NOBODY : predecessor->st_serialno, uctx);
	...
	/* SA out */
	{
		...
		if (!out_sa(&md.rbody, st->st_sadb, st, TRUE, INITIATOR, FALSE, np))
		{
		...
		clonetochunk(st->st_p1isa, sa_start, md.rbody.cur - sa_start, "sa in main_outI1");
	}

	if (SEND_PLUTO_VID || c->spd.this.cert.type == CERT_PGP)
	{
		char *vendorid = (c->spd.this.cert.type == CERT_PGP) ? pgp_vendorid : pluto_vendorid;
		int np = --numvidtosend > 0 ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE;

		if (!out_generic_raw(np, &isakmp_vendor_id_desc, &md.rbody, vendorid, strlen(vendorid), "Vendor ID"))
			return STF_INTERNAL_ERROR;
	}
	...
	clonetochunk(st->st_tpacket, reply_stream.start, pbs_offset(&reply_stream), "reply packet for main_outI1");
...
tpm_ignore:
	...
	if (predecessor != NULL)
	{
		...
		whack_log(RC_NEW_STATE + STATE_MAIN_I1, "%s: initiate, replacing #%lu", enum_name(&state_names, st->st_state), predecessor->st_serialno);
	}
	else
	{
		whack_log(RC_NEW_STATE + STATE_MAIN_I1, "%s: initiate", enum_name(&state_names, st->st_state));
	}
	reset_cur_state();
	return STF_OK;
}
...
void main_mode_hash_body(struct state *st, bool hashi /* Initiator? */
												 ...
												 ,
												 struct hmac_ctx *ctx, hash_update_t hash_update_void UNUSED)
#else
void main_mode_hash_body(struct state *st, bool hashi /* Initiator? */
												 ,
												 const pb_stream *idpl /* ID payload, as PBS */
...
#define hash_update_chunk(ctx, ch) hash_update((ctx), (ch).ptr, (ch).len)
	...
	DBG(DBG_CRYPT, DBG_log("hashing %lu bytes of SA", (unsigned long)(st->st_p1isa.len - sizeof(struct isakmp_generic))));

	/* SA_b */
#ifdef HAVE_LIBNSS
	hmac_update(ctx, st->st_p1isa.ptr + sizeof(struct isakmp_generic), st->st_p1isa.len - sizeof(struct isakmp_generic));
#else
	hash_update(ctx, st->st_p1isa.ptr + sizeof(struct isakmp_generic), st->st_p1isa.len - sizeof(struct isakmp_generic));
#endif
	 ...
	 */
#ifdef HAVE_LIBNSS
	hmac_update(ctx, idpl->start + sizeof(struct isakmp_generic), pbs_offset(idpl) - sizeof(struct isakmp_generic));
#else
	hash_update(ctx, idpl->start + sizeof(struct isakmp_generic), pbs_offset(idpl) - sizeof(struct isakmp_generic));
#endif
...
main_mode_hash(struct state *st, u_char *hash_val /* resulting bytes */
							 ,
 ...
 */
size_t
RSA_sign_hash(struct connection *c, u_char sig_val[RSA_MAX_OCTETS], const u_char *hash_val, size_t hash_len)
{
	size_t sz = 0;
...
static stf_status
RSA_check_signature(struct state *st, const u_char hash_val[MAX_DIGEST_LEN], size_t hash_len, const pb_stream *sig_pbs
#ifdef USE_KEYRR
										,
...
{
	return check_signature_gen(st->st_connection, st, hash_val, hash_len, sig_pbs
#ifdef USE_KEYRR
														 ,
														 keys_from_dns
#endif
...
bool encrypt_message(pb_stream *pbs, struct state *st)
{
	...
	DBG(DBG_CRYPT, DBG_log("encrypting %d using %s", (unsigned int)enc_len, enum_show(&oakley_enc_names, st->st_oakley.encrypter->common.ikev1_algo_id)));
...
main_inI1_outR1(struct msg_digest *md)
{
	...
	if (c == NULL)
	{
		...
		{
			struct connection *d;
			d = find_host_connection(ANY_MATCH, &md->iface->ip_addr, pluto_port500, KH_ANY, (ip_address *)NULL, md->sender_port, policy, POLICY_IKEV1_DISABLE, &policy_hint);
			...
			for (; d != NULL; d = d->IPhp_next)
			{
				...
				else
				{
					...
					if (addrinsubnet(&md->sender, &d->spd.that.client) && (c == NULL || !subnetinsubnet(&c->spd.that.client, &d->spd.that.client)))
						c = d;
				}
			}
		}
		...
		if (c == NULL)
		{
			loglog(RC_LOG_SERIOUS, "initial Main Mode message received on %s:%u"
						 ...
						 ip_str(&md->iface->ip_addr), ntohs(portof(&md->iface->ip_addr)), (policy != LEMPTY) ? " with policy=" : "", (policy != LEMPTY) ? bitnamesof(sa_policy_bit_names, policy) : "");
		...
		else
		{
			 ...
			 */
			DBG(DBG_CONTROL, DBG_log("instantiating \"%s\" for initial Main Mode message received on %s:%u", c->name, ip_str(&md->iface->ip_addr), pluto_port500));
	...
	{
		openswan_log("responding to Main Mode from unknown peer %s:%u", ip_str(&c->spd.that.host_addr), c->spd.that.host_port);
	}
	else if (c->kind == CK_INSTANCE)
	{
		openswan_log("responding to Main Mode from unknown peer %s", ip_str(&c->spd.that.host_addr));
	}
	else
	{
	...
	if (SEND_PLUTO_VID || openpgp_peer)
	{
		...
		if (!out_generic_raw(next, &isakmp_vendor_id_desc, &md->rbody, vendorid, strlen(vendorid), "Vendor ID"))
			return STF_INTERNAL_ERROR;
	}
...
#endif
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT, DBG_log("sender checking NAT-T: %d and %d", nat_traversal_enabled, md->quirks.nat_traversal_vid));
...
static stf_status
main_inR1_outI2_tail(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r);

static void
main_inR1_outI2_continue(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r, err_t ugh)
{
	...
	DBG(DBG_CONTROLMORE, DBG_log("main inR1_outI2: calculated ke+nonce, sending I2"));
...
main_inR1_outI2(struct msg_digest *md)
{
	...
	/* verify echoed SA */
	{
		...
		RETURN_STF_FAILURE(parse_isakmp_sa_body(&sapd->pbs, &sapd->payload.sa, NULL, TRUE, st));
	}
	...
	{
		st->hidden_variables.st_nat_traversal = nat_traversal_vid_to_method(md->quirks.nat_traversal_vid);
		openswan_log("enabling possible NAT-traversal with method %s", bitnamesof(natt_type_bitnames, st->hidden_variables.st_nat_traversal));
	}
#endif

	{
		struct ke_continuation *ke = alloc_thing(struct ke_continuation, "outI2 KE");
		ke->md = md;
...
bool ship_KE(struct state *st, struct pluto_crypto_req *r, chunk_t *g, pb_stream *outs, u_int8_t np)
{
	unpack_KE(st, r, g);
...
main_inR1_outI2_tail(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r)
{
...
#ifdef DEBUG
	/* Ni out */
	if (!ship_nonce(&st->st_ni, r, &md->rbody, (cur_debugging & IMPAIR_BUST_MI2) ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE, "Ni"))
		return STF_INTERNAL_ERROR;
	...
	if (cur_debugging & IMPAIR_BUST_MI2)
	{
		...
		if (!out_generic(ISAKMP_NEXT_NONE, &isakmp_vendor_id_desc, &md->rbody, &vid_pbs))
			return STF_INTERNAL_ERROR;
 ...
 */
static stf_status
main_inI2_outR2_tail(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r);

static void
main_inI2_outR2_continue(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r, err_t ugh)
{
	struct ke_continuation *ke = (struct ke_continuation *)pcrc;
	struct msg_digest *md = ke->md;
	struct state *const st = md->st;
	...
	DBG(DBG_CONTROLMORE, DBG_log("main inI2_outR2: calculated ke+nonce, sending R2"));
...
main_inI2_outR2(struct msg_digest *md)
{
...
#ifdef NAT_TRAVERSAL
	DBG(DBG_NATT, DBG_log("inI2: checking NAT-T: %d and %d", nat_traversal_enabled, st->hidden_variables.st_nat_traversal));
	...
	{
		struct ke_continuation *ke = alloc_thing(struct ke_continuation, "inI2_outR2 KE");
...
static void
main_inI2_outR2_calcdone(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r, err_t ugh)
{
	struct dh_continuation *dh = (struct dh_continuation *)pcrc;
	struct state *st;

	DBG(DBG_CONTROLMORE, DBG_log("main inI2_outR2: calculated DH finished"));
...
main_inI2_outR2_tail(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r)
{
...
#else
	/* Nr out */
	if (!ship_nonce(&st->st_nr, r, &md->rbody, (send_cr) ? ISAKMP_NEXT_CR : ISAKMP_NEXT_NONE, "Nr"))
		return STF_INTERNAL_ERROR;
#endif
	...
	if (send_cr)
	{
		...
		{
			if (!build_and_ship_CR(CERT_X509_SIGNATURE, st->st_connection->spd.that.ca, &md->rbody, ISAKMP_NEXT_NONE))
				return STF_INTERNAL_ERROR;
		}
		else
		{
			...
			if (collect_rw_ca_candidates(md, &ca))
			{
				...
				{
					if (!build_and_ship_CR(CERT_X509_SIGNATURE, gn->name, &md->rbody, gn->next == NULL ? ISAKMP_NEXT_NONE : ISAKMP_NEXT_CR))
						return STF_INTERNAL_ERROR;
				}
			...
			{
				if (!build_and_ship_CR(CERT_X509_SIGNATURE, empty_chunk, &md->rbody, ISAKMP_NEXT_NONE))
					return STF_INTERNAL_ERROR;
			}
		}
	}
	 ...
	 */
	{
		 ...
		 */
		struct dh_continuation *dh = alloc_thing(struct dh_continuation, "main_inI2_outR2_tail");
		stf_status e;
...
static void
doi_log_cert_thinking(struct msg_digest *md UNUSED, u_int16_t auth, enum ipsec_cert_type certtype, enum certpolicy policy, bool gotcertrequest, bool send_cert)
{
	DBG(DBG_CONTROL, DBG_log("thinking about whether to send my certificate:"));

	DBG(DBG_CONTROL, DBG_log("  I have RSA key: %s cert.type: %s ", enum_show(&oakley_auth_names, auth), enum_show(&cert_type_names, certtype)));

	DBG(DBG_CONTROL, DBG_log("  sendcert: %s and I did%s get a certificate request ", enum_show(&certpolicy_type_names, policy), gotcertrequest ? "" : " not"));
 ...
 */
static stf_status
main_inR2_outI3_continue(struct msg_digest *md, struct pluto_crypto_req *r)
{
	struct state *const st = md->st;
	 ...
	 * to always send one.
	 */
	send_cert = st->st_oakley.auth == OAKLEY_RSA_SIG && mycert.type != CERT_NONE && ((st->st_connection->spd.this.sendcert == cert_sendifasked && st->hidden_variables.st_got_certrequest) || st->st_connection->spd.this.sendcert == cert_alwayssend || st->st_connection->spd.this.sendcert == cert_forcedtype);
	...
	DBG(DBG_CONTROL, DBG_log(" I am %ssending a certificate request", send_cr ? "" : "not "));
	...
	/* IDii out */
	{
		...
		if (!out_struct(&id_hd, &isakmp_ipsec_identification_desc, &md->rbody, &id_pbs) || !out_chunk(id_b, &id_pbs, "my identity"))
			return STF_INTERNAL_ERROR;
	...
	if (send_cr)
	{
		...
		if (!build_and_ship_CR(mycert.type, st->st_connection->spd.that.ca, &md->rbody, ISAKMP_NEXT_SIG))
			return STF_INTERNAL_ERROR;
	}
	...
	/* HASH_I or SIG_I out */
	{
		...
		{
			/* HASH_I out */
			if (!out_generic_raw(ISAKMP_NEXT_NONE, &isakmp_hash_desc, &md->rbody, hash_val, hash_len, "HASH_I"))
				return STF_INTERNAL_ERROR;
		}
		else
		{
			...
			size_t sig_len = RSA_sign_hash(st->st_connection, sig_val, hash_val, hash_len);
			...
			if (!out_generic_raw(ISAKMP_NEXT_NONE, &isakmp_signature_desc, &md->rbody, sig_val, sig_len, "SIG_I"))
				return STF_INTERNAL_ERROR;
		}
	}
...
static void
main_inR2_outI3_cryptotail(struct pluto_crypto_req_cont *pcrc, struct pluto_crypto_req *r, err_t ugh)
{
	struct dh_continuation *dh = (struct dh_continuation *)pcrc;
	...
	DBG(DBG_CONTROLMORE, DBG_log("main inR2_outI3: calculated DH, sending I3"));
...
main_inR2_outI3(struct msg_digest *md)
{
	...
	return start_dh_secretiv(&dh->dh_pcrc, st, st->st_import, INITIATOR, st->st_oakley.group->group);
}
 ...
 */
stf_status
oakley_id_and_auth(struct msg_digest *md, bool initiator /* are we the Initiator? */
									 ,
...
)
{
	...
	case OAKLEY_RSA_SIG:
		r = RSA_check_signature(st, hash_val, hash_len, &md->chain[ISAKMP_NEXT_SIG]->pbs
#ifdef USE_KEYRR
														,
		...
		if (r == STF_SUSPEND)
		{
			...
			struct key_continuation *nkc = alloc_thing(struct key_continuation, "key continuation");
			...
			case kos_null:
				...
				nkc->failure_ok = TRUE;
#endif
				ugh = start_adns_query(&st->st_connection->spd.that.id, &st->st_connection->spd.that.id /* SG itself */
															 ,
			...
			case kos_his_txt:
				...
				ugh = start_adns_query(&st->st_connection->spd.that.id, NULL /* no sgw for KEY */
															 ,
...
main_id_and_auth(struct msg_digest *md, bool initiator /* are we the Initiator? */
								 ,
								 cont_fn_t cont_fn /* continuation function */
								 ,
...
static stf_status
main_inI3_outR3_tail(struct msg_digest *md, struct key_continuation *kc)
{
	struct state *const st = md->st;
	...
	/* IDir out */
	{
		...
		build_id_payload(&id_hd, &id_b, &st->st_connection->spd.this);
		id_hd.isaiid_np = (send_cert) ? ISAKMP_NEXT_CERT : auth_payload;
		if (!out_struct(&id_hd, &isakmp_ipsec_identification_desc, &md->rbody, &r_id_pbs) || !out_chunk(id_b, &r_id_pbs, "my identity"))
			return STF_INTERNAL_ERROR;
	...
	/* HASH_R or SIG_R out */
	{
		...
		{
			/* HASH_R out */
			if (!out_generic_raw(np, &isakmp_hash_desc, &md->rbody, hash_val, hash_len, "HASH_R"))
				return STF_INTERNAL_ERROR;
		}
		else
		{
			...
			if (!out_generic_raw(np, &isakmp_signature_desc, &md->rbody, sig_val, sig_len, "SIG_R"))
				return STF_INTERNAL_ERROR;
		}
	}
...
static stf_status
main_inR3_tail(struct msg_digest *md, struct key_continuation *kc)
{
	struct state *const st = md->st;
...
stf_status
send_isakmp_notification(struct state *st, u_int16_t type, const void *data, size_t len)
{
	msgid_t msgid;
	pb_stream rbody;
									...
									u_char *spi, size_t spisize, u_char protoid)
{
	...
	case PAYLOAD_MALFORMED:
		...
		{
			openswan_log("too many (%d) malformed payloads. Deleting state", sndst->hidden_variables.st_malformed_sent);
			delete_state(sndst);
			return;
		}
	...
	openswan_log("sending %snotification %s to %s:%u", encst ? "encrypted " : "", enum_name(&ipsec_notification_names, type), ip_str(&sndst->st_remoteaddr), sndst->st_remoteport);
...
void ikev1_delete_out(struct state *st)
{
	...
	if (isakmp_sa)
	{
		...
		if (!out_struct(&isad, &isakmp_delete_desc, &r_hdr_pbs, &del_pbs) || !out_raw(&isakmp_spi, (2 * COOKIE_SIZE), &del_pbs, "delete payload"))
			impossible();
	...
	else
	{
		while (ns != said)
		{
			...
			isad.isad_nospi = 1;
			if (!out_struct(&isad, &isakmp_delete_desc, &r_hdr_pbs, &del_pbs) || !out_raw(&ns->spi, sizeof(ipsec_spi_t), &del_pbs, "delete payload"))
				impossible();
			close_output_pbs(&del_pbs);
		}
	}
 ...
 */
void accept_delete(struct state *st, struct msg_digest *md, struct payload_digest *p)
{
	...
	case PROTO_IPCOMP:
		...
		return;
	default:
		loglog(RC_LOG_SERIOUS, "ignoring Delete SA payload: unknown Protocol ID (%s)", enum_show(&protocol_names, d->isad_protoid));
		return;
	}

	if (d->isad_spisize != sizespi)
	{
		loglog(RC_LOG_SERIOUS, "ignoring Delete SA payload: bad SPI size (%d) for %s", d->isad_spisize, enum_show(&protocol_names, d->isad_protoid));
		return;
	}

	if (pbs_left(&p->pbs) != d->isad_nospi * sizespi)
	{
		loglog(RC_LOG_SERIOUS, "ignoring Delete SA payload: invalid payload size");
		return;
	}
	...
	for (i = 0; i < d->isad_nospi; i++)
	{
		...
		if (d->isad_protoid == PROTO_ISAKMP)
		{
			...
			struct state *dst = find_state_ikev1(spi /*iCookie*/
																					 ...
																					 ,
																					 &st->st_connection->spd.that.host_addr, MAINMODE_MSGID);
		...
		else
		{
			 ...
			 */
			bool bogus;
			struct state *dst = find_phase2_state_to_delete(st, d->isad_protoid, *(ipsec_spi_t *)spi /* network order */
																											,
																											&bogus);

			if (dst == NULL)
			{
				loglog(RC_LOG_SERIOUS, "ignoring Delete SA payload: %s SA(0x%08lx) not found (%s)", enum_show(&protocol_names, d->isad_protoid), (unsigned long)ntohl((unsigned long)*(ipsec_spi_t *)spi), bogus ? "our SPI - bogus implementation" : "maybe expired");
			}
			else
			{
...
#endif
				if (rc->newest_ipsec_sa == dst->st_serialno && (rc->policy & POLICY_UP))
				{
...
#define DELETE_SA_DELAY EVENT_RETRANSMIT_DELAY_0
					if (dst->st_event != NULL && dst->st_event->ev_type == EVENT_SA_REPLACE && dst->st_event->ev_time <= DELETE_SA_DELAY + now())
					{
				...
				else
				{
					loglog(RC_LOG_SERIOUS, "received Delete SA(0x%08lx) payload: "
								 ...
								 (unsigned long)ntohl((unsigned long)*(ipsec_spi_t *)spi), dst->st_serialno);
					delete_state(dst);
				}
