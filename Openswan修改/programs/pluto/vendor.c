...
init_pluto_vendorid(void)
{
	...
	osMD5Update(&hc, (const unsigned char *)compile_time_interop_options, strlen(compile_time_interop_options));
	osMD5Final(hash, &hc);
	...
	memset(pluto_vendorid + 3 + MD5_DIGEST_SIZE, '\0', PLUTO_VENDORID_SIZE - 3 - MD5_DIGEST_SIZE);
#endif
...
void init_vendorid(void)
{
	...
	for (vid = _vid_tab; vid->id; vid++)
	{
		if (vid->flags & VID_SELF)
		{
			...
			d = alloc_bytes(strlen(vid->descr) + 4 + strlen(ipsec_version_code()) + strlen(compile_time_interop_options), "self-vendor ID");
			sprintf(d, "%s %s %s", vid->descr, ipsec_version_code(), compile_time_interop_options);
 ...
 * @return void
 */
static void handle_known_vendorid(struct msg_digest *md, const char *vidstr, size_t len, struct vid_struct *vid, struct state *st UNUSED)
{
	char vid_dump[128];
	int vid_usefull = 0;
	size_t i, j;
	...
	case VID_NATT_RFC:
		...
		{
			loglog(RC_LOG_SERIOUS, "received Vendor ID payload [%s] meth=%d, "
		...
		else
		{
			...
			{
				loglog(RC_LOG_SERIOUS, "received Vendor ID payload [%s] method set to=%d ", vid->descr, vid->id);
				md->quirks.nat_traversal_vid = vid->id;
				return;
			}
			else
