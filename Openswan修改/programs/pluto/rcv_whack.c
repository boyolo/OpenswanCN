...
key_add_request(const struct whack_message *msg)
{
	...
	else
	{
		...
		if (msg->keyval.len == 0)
		{
			struct key_add_common *oc = alloc_thing(struct key_add_common, "key add common things");
			enum key_add_attempt kaa;
			...
			for (kaa = ka_TXT; kaa != ka_roof; kaa++)
			{
				struct key_add_continuation *kc = alloc_thing(struct key_add_continuation, "key add continuation");
				...
				{
				case ka_TXT:
					ugh = start_adns_query(&keyid, &keyid /* same */
																 ,
																 ns_t_txt, key_add_continue, &kc->ac);
					break;
#ifdef USE_KEYRR
				case ka_KEY:
					ugh = start_adns_query(&keyid, NULL, ns_t_key, key_add_continue, &kc->ac);
					break;
#endif /* USE_KEYRR */
				default:
		...
		else
		{
			...
			ugh = add_public_key(&keyid, DAL_LOCAL, msg->pubkey_alg, &msg->keyval, &pluto_pubkeys);
			if (ugh != NULL)
...
void whack_process(int whackfd, struct whack_message msg)
{
	...
	if (msg.whack_options)
	{
		...
		case WHACK_ADJUSTOPTIONS:
			...
			if (msg.name == NULL)
			{
				...
				DBG(DBG_CONTROL, DBG_log("base debugging = %s", bitnamesof(debug_bit_names, msg.debugging)));
			...
			else if (!msg.whack_connection)
			{
				...
				if (c != NULL)
				{
					...
					DBG(DBG_CONTROL, DBG_log("\"%s\" extra_debugging = %s", c->name, bitnamesof(debug_bit_names, c->extra_debugging)));
				}
			}
#endif
			break;
	...
	if (msg.whack_deletestate)
	{
		...
		if (st == NULL)
		{
			loglog(RC_UNKNOWN_NAME, "no state #%lu to delete", (long unsigned int)msg.whack_deletestateno);
		}
		else
		{
			delete_state(st);
	...
	if (msg.whack_route)
	{
		...
		else
		{
			...
			if (c != NULL)
			{
				...
				if (!oriented(*c))
					whack_log(RC_ORIENT, "we cannot identify ourselves with either end of this connection");
	...
	if (msg.whack_initiate)
	{
		if (!listening)
			...
			initiate_connection(msg.name, msg.whack_async ? NULL_FD : dup_any(whackfd), msg.debugging, pcim_demand_crypto);
	}
	...
	if (msg.whack_oppo_initiate)
	{
		if (!listening)
			...
			(void)initiate_ondemand(&msg.oppo_my_client, &msg.oppo_peer_client, 0, FALSE, msg.whack_async ? NULL_FD : dup_any(whackfd), NULL, "whack");
	}
...
void whack_handle(int whackctlfd)
{
	...
	/* sanity check message */
	{
		...
		else if (msg.magic != WHACK_MAGIC)
		{
			...
			{
				ugh = builddiag("ignoring message from whack with bad magic %08x; should be %08x; Mismatched versions of userland tools and PLUTO code.", msg.magic, WHACK_MAGIC);
			}
		}
 ...
 */
bool whack_prompt_for(int whackfd, const char *prompt1, const char *prompt2, bool echo, char *ansbuf, size_t ansbuf_len)
{
	...
	whack_log(echo ? RC_XAUTHPROMPT : RC_ENTERSECRET, "%s prompt for %s:", prompt1, prompt2);
