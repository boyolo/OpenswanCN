...
bool ikev2_out_attr(unsigned int type, unsigned long val, struct_desc *attr_desc, enum_names **attr_val_descs USED_BY_DEBUG, pb_stream *pbs)
{
	...
	DBG(
			DBG_EMITTING,
			if (type < ikev2_trans_attr_val_descs_size && (d = attr_val_descs[type]) != NULL) {
...
bool ikev2_out_sa(pb_stream *outs, unsigned int protoid, struct db_sa *sadb, struct state *st, bool parentSA, u_int8_t np)
{
	pb_stream sa_pbs;
	bool ret = FALSE;
	unsigned int pc_cnt;
	...
	for (pc_cnt = 0; pc_cnt < sadb->prop_disj_cnt; pc_cnt++)
	{
		...
		for (pr_cnt = 0; pr_cnt < vp->prop_cnt; pr_cnt++)
		{
			...
			if (p.isap_spisize > 0)
			{
				...
				{
					if (!out_raw(&st->st_esp.our_spi, 4, &t_pbs, "our spi"))
						return STF_INTERNAL_ERROR;
				}
			}
			...
			for (ts_cnt = 0; ts_cnt < vpc->trans_cnt; ts_cnt++)
			{
				...
				for (attr_cnt = 0; attr_cnt < tr->attr_cnt; attr_cnt++)
				{
					...
					ikev2_out_attr(attr->ikev2, attr->val, &ikev2_trans_attr_desc, ikev2_trans_attr_val_descs, &at_pbs);
				}
 ...
 */
bool spdb_v2_match_parent(struct db_sa *sadb, unsigned propnum, unsigned encr_transform, int encr_keylen, unsigned integ_transform, unsigned prf_transform, unsigned dh_transform)
{
		...
		for (pj_cnt = 0; pj_cnt < pd->prop_cnt; pj_cnt++)
		{
			...
			for (tr_cnt = 0; tr_cnt < pj->trans_cnt; tr_cnt++)
			{
				...
				if (dh_matched && prf_matched && integ_matched && encr_matched)
				{
					if (DBGP(DBG_CONTROL))
					{
						DBG_log("selected proposal %u encr=%s[%d] integ=%s prf=%s modp=%s", propnum, enum_name(&trans_type_encr_names, encrid), encr_keylen, enum_name(&trans_type_integ_names, integid), enum_name(&trans_type_prf_names, prfid), enum_name(&oakley_group_names, dhid));
					}
			...
			if (DBGP(DBG_CONTROLMORE))
			{
				...
				DBG_log("proposal %u %6s encr= (policy:%20s[%d] vs offered:%s[%d]) [%u,%u]", propnum, encr_matched ? "succ" : "failed", enum_name(&trans_type_encr_names, encrid), encr_keylen, enum_show(&trans_type_encr_names, encr_transform), encr_keylen_policy, pd_cnt, attempt++);
				DBG_log("proposal %u %6s integ=(policy:%20s vs offered:%s)", propnum, integ_matched ? "succ" : "failed", enum_name(&trans_type_integ_names, integid), enum_show(&trans_type_integ_names, integ_transform));
				DBG_log("proposal %u %6s prf=  (policy:%20s vs offered:%s)", propnum, prf_matched ? "succ" : "failed", enum_name(&trans_type_prf_names, prfid), enum_show(&trans_type_prf_names, prf_transform));
				DBG_log("proposal %u %6s dh=   (policy:%20s vs offered:%s)", propnum, dh_matched ? "succ" : "failed", enum_name(&oakley_group_names, dhid), enum_show(&oakley_group_names, dh_transform));
			}
		}
	}
	return FALSE;
...
static bool
ikev2_match_transform_list_parent(struct db_sa *sadb, unsigned int propnum, struct ikev2_transform_list *itl, struct trans_attrs *winning)
{
...
static stf_status
ikev2_process_transforms(struct ikev2_prop *prop, pb_stream *prop_pbs, struct ikev2_transform_list *itl)
{
	...
	while (prop->isap_numtrans-- > 0)
	{
		...
		{
			if (!in_struct(&attr, &ikev2_trans_attr_desc, &trans_pbs, &attr_pbs))
				return BAD_PROPOSAL_SYNTAX;
...
ikev2_emit_winning_sa(
		struct state *st, pb_stream *r_sa_pbs, struct trans_attrs ta, bool parentSA, struct ikev2_prop winning_prop)
{
	...
	if (!out_struct(&r_proposal, &ikev2_prop_desc, r_sa_pbs, &r_proposal_pbs))
		impossible();
	...
	if (!out_struct(&r_trans, &ikev2_trans_desc, &r_proposal_pbs, &r_trans_pbs))
		impossible();
	if (ta.encrypter && ta.encrypter->keyminlen != ta.encrypter->keymaxlen)
		ikev2_out_attr(IKEv2_KEY_LENGTH, ta.enckeylen, &ikev2_trans_attr_desc, ikev2_trans_attr_val_descs, &r_trans_pbs);
...
)
{
	...
	while (np == ISAKMP_NEXT_P)
	{
		...
		{
			loglog(RC_LOG_SERIOUS, "invalid SPI size (%u) in PARENT_SA Proposal", (unsigned)proposal.isap_spisize);
			return INVALID_SPI;
		}
		...
		{
			stf_status ret = ikev2_process_transforms(&proposal, &proposal_pbs, itl);
			if (ret != STF_OK)
				return ret;
		}
		...
		if (ikev2_match_transform_list_parent(st->st_sadb, proposal.isap_propnum, itl, &ta))
		{
	 ...
	 * winning value.
	 */
	ta.encrypter = (struct ike_encr_desc *)ike_alg_ikev2_find(IKEv2_TRANS_TYPE_ENCR, ta.encrypt, ta.enckeylen);
	...
	if (r_sa_pbs != NULL)
	{
		return ikev2_emit_winning_sa(st, r_sa_pbs, ta, /*parentSA*/ TRUE, winning_prop);
	}
	return NOTHING_WRONG;
}

bool spdb_v2_match_child(struct db_sa *sadb, unsigned propnum, unsigned encr_transform, int encr_keylen, unsigned integ_transform, unsigned esn_transform)
{
	struct db_v2_prop *pd;
	unsigned int pd_cnt;
	...
	for (pd_cnt = 0; pd_cnt < sadb->prop_disj_cnt; pd_cnt++)
	{
		...
		for (pc_cnt = 0; pc_cnt < pd->prop_cnt; pc_cnt++)
		{
			...
			if (DBGP(DBG_CONTROLMORE))
			{
				DBG_log("proposal %u,%u %s encr= (policy:%20s[%d] vs offered:%s[%d])", propnum, pc_cnt, encr_matched ? "match " : "failed", enum_name(&trans_type_encr_names, encrid), encr_keylen, enum_name(&trans_type_encr_names, encr_transform), encr_keylen_policy);
				DBG_log("proposal %u,%u %s integ=(policy:%20s vs offered:%s)", propnum, pc_cnt, integ_matched ? "match " : "failed", enum_name(&trans_type_integ_names, integid), enum_name(&trans_type_integ_names, integ_transform));
				DBG_log("proposal %u,%u %s esn=  (policy:%20s vs offered:%s)", propnum, pc_cnt, esn_matched ? "match " : "failed", enum_name(&trans_type_esn_names, esnid), enum_name(&trans_type_esn_names, esn_transform));
			}
		}
	}
	return FALSE;
...
static bool
ikev2_match_transform_list_child(struct db_sa *sadb, unsigned int propnum, struct ikev2_transform_list *itl, struct trans_attrs *winning)
{
	if (itl->encr_trans_next < 1)
	{
		openswan_log("ignored proposal %u with no cipher transforms",
...
)
{
	...
	while (np == ISAKMP_NEXT_P)
	{
		...
		case PROTO_IPSEC_ESP:
			...
			{
				unsigned int spival;
				if (!in_raw(&spival, proposal.isap_spisize, &proposal_pbs, "CHILD SA SPI"))
					return PAYLOAD_MALFORMED;

				DBG(DBG_PARSING, DBG_log("SPI received: %08x", ntohl(spival)));
				itl->spi_values[itl->spi_values_next++] = spival;
			}
			else
			{
				loglog(RC_LOG_SERIOUS, "invalid SPI size (%u) in CHILD_SA Proposal", (unsigned)proposal.isap_spisize);
				return INVALID_SPI;
			}
			break;

		default:
			loglog(RC_LOG_SERIOUS, "unexpected Protocol ID (%s) found in PARENT_SA Proposal", enum_show(&protocol_names, proposal.isap_protoid));
		...
		if (ikev2_match_transform_list_child(p2alg, proposal.isap_propnum, itl, &ta))
		{
	...
	if (r_sa_pbs != NULL)
	{
		return ikev2_emit_winning_sa(st, r_sa_pbs, ta, /*parentSA*/ FALSE, winning_prop);
	}

	return NOTHING_WRONG;
}

stf_status ikev2_emit_ipsec_sa(struct msg_digest *md, pb_stream *outpbs, unsigned int np, struct connection *c, lset_t policy UNUSED)
{
	int proto;
	struct db_sa *p2alg;
