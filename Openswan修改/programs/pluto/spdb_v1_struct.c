...
bool out_attr(int type, unsigned long val, struct_desc *attr_desc, enum_names **attr_val_descs USED_BY_DEBUG, pb_stream *pbs)
{
    ...
    else
    {
        ...
        if (!out_struct(&attr, attr_desc, pbs, &val_pbs) || !out_raw(&nval, sizeof(nval), &val_pbs, "long attribute value"))
            return FALSE;
...
struct db_sa *
ikev1_alg_makedb(lset_t policy, struct alg_info_ike *ei, bool oneproposal UNUSED, enum phase1_role role)
{
    struct db_sa *sadb;
    char info_buf[1024];
...
bool extrapolate_v1_from_v2(struct db_sa *sadb, lset_t policy, enum phase1_role role)
{
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        for (prop_disj = 0; prop_disj < sadb->prop_disj_cnt; prop_disj++)
        {
...
#if EXTRAPOLATE_DEBUG
            /* enable this when debugging problems with cur_combo */
            DBG_log("%u: %u disj_cnt: %d/%d conj: 0/%d", pass, cur_combo, prop_disj, sadb->prop_disj_cnt, pd->prop_cnt);
#endif
            ...
            for (prop_conj = 0; prop_conj < pd->prop_cnt; prop_conj++)
            {
...
#if EXTRAPOLATE_DEBUG
                /* enable this when debugging problems with cur_combo */
                DBG_log("%u: %u disj_cnt: %d/%d conj: %d/%d trans: 0/%d", pass, cur_combo, prop_disj, sadb->prop_disj_cnt, prop_conj, pd->prop_cnt, pc->trans_cnt);
#endif
...
#if EXTRAPOLATE_DEBUG
                    /* enable this when debugging problems with cur_combo */
                    DBG_log("%u: %u disj_cnt: %d/%d conj: %d/%d trans: %d/%d type: %d", pass, cur_combo, prop_disj, sadb->prop_disj_cnt, prop_conj, pd->prop_cnt, trans_i, pc->trans_cnt, tr->transform_type);
#endif

                    /* if this is an invalid transform type */
                    if (tr->transform_type >= IKEv2_TRANS_TYPE_COUNT)
                        continue;

                    /* IKEv1 does not negotiate PRF, so ignore options like that */
                    if (tr->transform_type == IKEv2_TRANS_TYPE_PRF)
                        continue;

#if EXTRAPOLATE_DEBUG
                    /* enable this when debugging problems with cur_combo */
                    DBG_log("%u: %u A: %u,%u,%u, noticing type[%u, %s]=>%d (vs: %d)", pass, cur_combo, prop_disj, prop_conj, trans_i, tr->transform_type, enum_name(&trans_type_names, tr->transform_type), tr->value, transform_values[tr->transform_type]);
#endif
                ...
                /* now see if we have a new combination */
                {
                    ...
                    if ((pass == 0 && cur_combo == 0) || (pass == 1 && cur_combo == 0) || !matched)
                    {
                        /*
                        ...
                        DBG(DBG_EMITTING, DBG_log("pass: %u noticed combo %d/%d: dh:%d(%s) encr:%d(%s) integ:%d(%s)", pass, cur_combo, tot_combos, transform_values[IKEv2_TRANS_TYPE_DH], enum_name(&oakley_group_names, transform_values[IKEv2_TRANS_TYPE_DH]), transform_values[IKEv2_TRANS_TYPE_ENCR], enum_name(&trans_type_encr_names, transform_values[IKEv2_TRANS_TYPE_ENCR]), transform_values[IKEv2_TRANS_TYPE_INTEG], enum_name(&trans_type_integ_names, transform_values[IKEv2_TRANS_TYPE_INTEG])));
                        ...
                        case 1:
                        {
                            ...
                            {
#if EXTRAPOLATE_DEBUG
                                DBG_log("%d: <%d checking combo dh:%d:%d encr:%d:%d integ:%d:%d", combo_i, cur_combo, cur_dtf->group_transid, dtf[combo_i].group_transid, cur_dtf->encr_transid, dtf[combo_i].encr_transid, cur_dtf->integ_transid, dtf[combo_i].integ_transid);
#endif
                                if (cur_dtf->group_transid == dtf[combo_i].group_transid && cur_dtf->encr_transid == dtf[combo_i].encr_transid && cur_dtf->integ_transid == dtf[combo_i].integ_transid)
                                {
                                    /* it is duplicate */
#if EXTRAPOLATE_DEBUG
                             ...
                             * so skip them.
                             */
                            if (v2tov1_encr(cur_dtf->encr_transid) == 0 || v2tov1_integ(cur_dtf->integ_transid) == 0)
                            {
                                DBG(DBG_EMITTING, DBG_log("encr or integ transid not available in IKEv1"));
                                continue;
                            }

                            DBG(DBG_EMITTING, DBG_log("new combo %d: dh:%d encr:%d integ:%d", cur_combo, cur_dtf->group_transid, cur_dtf->encr_transid, cur_dtf->integ_transid));
                            cur_dtf++;
                            *cur_dtf = *old_dtf;
                            ++cur_combo;
        ...
        if (pass == 0)
        {
            ...
            if ((transform_values[IKEv2_TRANS_TYPE_DH] != -1 || sadb->parentSA == FALSE) && transform_values[IKEv2_TRANS_TYPE_ENCR] != -1 && transform_values[IKEv2_TRANS_TYPE_INTEG] != -1)
            {
                tot_combos++;
            }
            ...
            if (transform_values[IKEv2_TRANS_TYPE_ENCR] == -1 || transform_values[IKEv2_TRANS_TYPE_INTEG] == -1)
            {
     ...
     */
    if (cur_combo == 0 || (cur_dtf->group_transid != dtf[cur_combo].group_transid || cur_dtf->encr_transid != dtf[cur_combo].encr_transid || cur_dtf->integ_transid != dtf[cur_combo].integ_transid))
    {
        cur_combo++;
    }
 ...
 *
 */
bool out_sa(pb_stream *outs, struct db_sa *sadb, struct state *st, bool phase_one_mode, enum phase1_role role, bool aggressive_mode UNUSED, u_int8_t np)
{
    pb_stream sa_pbs;
    unsigned int pcn;
    bool ret = FALSE;
    bool ah_spi_generated = FALSE, esp_spi_generated = FALSE, ipcomp_cpi_generated = FALSE;
    ...
    for (pcn = 0; pcn < sadb->prop_conj_cnt; pcn++)
    {
        ...
        for (pn = 0; pn < pc->prop_cnt; pn++)
        {
             ...
             * place.
             */
            tunnel_mode = (valid_prop_cnt == 1) && (st->st_policy & POLICY_TUNNEL);
             ...
             */
            {
                ...
                case PROTO_IPCOMP:
                     ...
                     */
                    if (!out_raw((u_char *)&st->st_ipcomp.our_spi + IPSEC_DOI_SPI_SIZE - IPCOMP_CPI_SIZE, IPCOMP_CPI_SIZE, &proposal_pbs, "CPI"))
                        return_on(ret, FALSE);
                    break;
                ...
                if (pi != NULL)
                {
                    ...
                    {
                        if (!get_ipsec_spi(pi, proto, st, tunnel_mode))
                        {
                            return FALSE;
                        }
                        *spi_generated = TRUE;
                    }
                    if (!out_raw((u_char *)&pi->our_spi, IPSEC_DOI_SPI_SIZE, &proposal_pbs, "SPI"))
                        return_on(ret, FALSE);
                }
            }
            ...
            for (tn = 0; tn != p->trans_cnt; tn++)
            {
                ...
                if (p->protoid != PROTO_IPCOMP && st->st_pfs_group != NULL)
                {
                    ...
                    out_attr(GROUP_DESCRIPTION, st->st_pfs_group->group, attr_desc, attr_val_descs, &trans_pbs);
                }
                ...
                if (phase_one_mode)
                {
                    out_attr(OAKLEY_LIFE_TYPE, OAKLEY_LIFE_SECONDS, attr_desc, attr_val_descs, &trans_pbs);
                    out_attr(OAKLEY_LIFE_DURATION, st->st_connection->sa_ike_life_seconds, attr_desc, attr_val_descs, &trans_pbs);
                }
                else
                {
                     ...
                     */
                    if (p->protoid != PROTO_IPCOMP || st->st_policy & POLICY_TUNNEL)
                    {
                        ...
                        out_attr(ENCAPSULATION_MODE, NAT_T_ENCAPSULATION_MODE(st, st->st_policy), attr_desc, attr_val_descs, &trans_pbs);
                    }
                    transmitted |= LELEM(SA_LIFE_TYPE);
                    out_attr(SA_LIFE_TYPE, SA_LIFE_TYPE_SECONDS, attr_desc, attr_val_descs, &trans_pbs);
                    transmitted |= LELEM(SA_LIFE_DURATION);
                    out_attr(SA_LIFE_DURATION, st->st_connection->sa_ipsec_life_seconds, attr_desc, attr_val_descs, &trans_pbs);
#ifdef HAVE_LABELED_IPSEC
                ...
                for (an = 0; an < t->attr_cnt; an++)
                {
                    ...
                    if (0)
                    {
                        DBG(DBG_EMITTING, DBG_log("emitting transform for %s with value %u", enum_name(thing, type), a->val));
                    }
...
decode_long_duration(pb_stream *pbs)
{
    ...
    if (pbs_left(pbs) > sizeof(val))
    {
        ...
        DBG(DBG_PARSING, DBG_log("   too large duration clamped to: %lu", (unsigned long)val));
    }
    else
    {
        /* decode number */
    ...
    struct state *st)           /* current state object */
{
    ...
    {
        loglog(RC_LOG_SERIOUS, "Proposal Payload must be alone in Oakley SA; found %s following Proposal", enum_show(&payload_names, proposal.isap_np));
        return PAYLOAD_MALFORMED;
    }

    if (proposal.isap_protoid != PROTO_ISAKMP)
    {
        loglog(RC_LOG_SERIOUS, "unexpected Protocol ID (%s) found in Oakley Proposal", enum_show(&protocol_names, proposal.isap_protoid));
    ...
    {
        loglog(RC_LOG_SERIOUS, "invalid SPI size (%u) in Oakley Proposal", (unsigned)proposal.isap_spisize);
        return INVALID_SPI;
    }

    if (selection && proposal.isap_notrans != 1)
    {
        loglog(RC_LOG_SERIOUS, "a single Transform is required in a selecting Oakley Proposal; found %u", (unsigned)proposal.isap_notrans);
    ...
    for (;;)
    {
        ...
        {
            loglog(RC_LOG_SERIOUS, "expected KEY_IKE but found %s in Oakley Transform", enum_show(&isakmp_transformid_names, trans.isat_transid));
        ...
        while (pbs_left(&trans_pbs) != 0)
        {
            ...
            if ((a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK) >= 32)
            {
                ...
                loglog(RC_LOG_SERIOUS, "ignoring unknown attribute %u in Oakley Transform %u", a.isaat_af_type, trans.isat_transnum);
                continue;
            }

            if (LHAS(seen_attrs, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK))
            {
                loglog(RC_LOG_SERIOUS, "repeated %s attribute in Oakley Transform %u", enum_show(&oakley_attr_names, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK), trans.isat_transnum);
                return BAD_PROPOSAL_SYNTAX;
            }
            ...
            case OAKLEY_HASH_ALGORITHM | ISAKMP_ATTR_AF_TV:
                ...
                {
                    ugh = builddiag("%s is not supported", enum_show(&oakley_hash_names, val));
                }
                break;
            ...
            case OAKLEY_AUTHENTICATION_METHOD | ISAKMP_ATTR_AF_TV:
            {
                ...
                psk:
                    ...
                    else
                    {
                        ...
                        {
                            char mid[IDTOA_BUF], hid[IDTOA_BUF];
                            ...
                            ugh = builddiag("Can't authenticate: no preshared key found for `%s' and `%s'", mid, hid);
                        }
                        ta.auth = val;
                    }
                    break;
                ...
                default:
                    ugh = builddiag("Pluto does not support %s authentication", enum_show(&oakley_auth_names, val));
                    break;
                }
            }
            break;
            ...
            case OAKLEY_LIFE_TYPE | ISAKMP_ATTR_AF_TV:
                ...
                case OAKLEY_LIFE_KILOBYTES:
                    ...
                    {
                        loglog(RC_LOG_SERIOUS, "attribute OAKLEY_LIFE_TYPE value %s repeated", enum_show(&oakley_lifetime_names, val));
                        return BAD_PROPOSAL_SYNTAX;
                    }
                    ...
                    break;
                default:
                    ugh = builddiag("unknown value %s", enum_show(&oakley_lifetime_names, val));
                    break;
                }
                break;
                ...
                case OAKLEY_LIFE_SECONDS:
                    if (val > OAKLEY_ISAKMP_SA_LIFETIME_MAXIMUM)
                        ugh = builddiag("peer requested %lu seconds"
                                        ...
                                        (long)val, OAKLEY_ISAKMP_SA_LIFETIME_MAXIMUM);
                    ta.life_seconds = val;
                    break;
                case OAKLEY_LIFE_KILOBYTES:
            ...
            if (ugh != NULL)
            {
                loglog(RC_LOG_SERIOUS, "%s.  Attribute %s", ugh, enum_show(&oakley_attr_names, attr));
                break;
            }
        }
        ...
        if (ugh == NULL)
        {
            ...
            {
                lset_t missing = ~seen_attrs & (LELEM(OAKLEY_ENCRYPTION_ALGORITHM) | LELEM(OAKLEY_HASH_ALGORITHM) | LELEM(OAKLEY_AUTHENTICATION_METHOD) | LELEM(OAKLEY_GROUP_DESCRIPTION));

                if (missing)
                {
                    loglog(RC_LOG_SERIOUS, "missing mandatory attribute(s) %s in Oakley Transform %u", bitnamesof(oakley_attr_bit_names, missing), trans.isat_transnum);
                    return BAD_PROPOSAL_SYNTAX;
                }
            }
        ...
        {
            loglog(RC_LOG_SERIOUS, "unexpected %s payload in Oakley Proposal", enum_show(&payload_names, proposal.isap_np));
            return BAD_PROPOSAL_SYNTAX;
        }
    }
...
bool init_am_st_oakley(struct state *st, lset_t policy)
{
    ...
    DBG(DBG_CONTROL, DBG_log("initiating aggressive mode with IKE=E=%d-H=%d-M=%d", enc->val, hash->val, grp->val));
...
static bool
parse_ipsec_transform(struct isakmp_transform *trans, struct ipsec_trans_attrs *attrs /* must be in IKEv2 number space */
                      ,
                      pb_stream *prop_pbs, pb_stream *trans_pbs, struct_desc *trans_desc, int previous_transnum /* or -1 if none */
                      ,
                      bool selection, bool is_last, bool is_ipcomp, struct state *st) /* current state object */
{
    lset_t seen_attrs = 0, seen_durations = 0;
    u_int16_t life_type;
    ...
    case ISAKMP_NEXT_NONE:
        ...
        break;
    default:
        loglog(RC_LOG_SERIOUS, "expecting Transform Payload, but found %s in Proposal", enum_show(&payload_names, trans->isat_np));
        return FALSE;
    }
    ...
    *attrs = null_ipsec_trans_attrs;
    ...
    while (pbs_left(trans_pbs) != 0)
    {
        ...
        if (LHAS(seen_attrs, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK))
        {
            loglog(RC_LOG_SERIOUS, "repeated %s attribute in IPsec Transform %u", enum_show(&ipsec_attr_names, a.isaat_af_type & ISAKMP_ATTR_RTYPE_MASK), trans->isat_transnum);
            return FALSE;
        }
        ...
        if (vdesc != NULL)
        {
            ...
            {
                loglog(RC_LOG_SERIOUS, "invalid value %u for attribute %s in IPsec Transform", (unsigned)val, enum_show(&ipsec_attr_names, a.isaat_af_type));
                return FALSE;
            }
            DBG(DBG_PARSING, if ((a.isaat_af_type & ISAKMP_ATTR_AF_MASK) == ISAKMP_ATTR_AF_TV)
                                 DBG_log("   [%u is %s]", (unsigned)val, enum_show(vdesc, val)));
        }
        ...
        case SA_LIFE_TYPE:
            ...
            if (LHAS(seen_durations, val))
            {
                loglog(RC_LOG_SERIOUS, "attribute SA_LIFE_TYPE value %s repeated in message", enum_show(&sa_lifetime_names, val));
                return FALSE;
            }
            seen_durations |= LELEM(val);
            life_type = val;
        ...
        case GROUP_DESCRIPTION:
            if (is_ipcomp)
            {
                 ...
                 */
                ipcomp_inappropriate = FALSE;
                loglog(RC_COMMENT, "IPCA (IPcomp SA) contains GROUP_DESCRIPTION."
        ...
        default:
            ...
            {
#endif
                loglog(RC_LOG_SERIOUS, "unsupported IPsec attribute %s", enum_show(&ipsec_attr_names, a.isaat_af_type));
                return FALSE;
#ifdef HAVE_LABELED_IPSEC
            }
#endif
        ...
        {
            loglog(RC_LOG_SERIOUS, "IPsec attribute %s inappropriate for IPCOMP", enum_show(&ipsec_attr_names, a.isaat_af_type));
            return FALSE;
        }
    }
    ...
    if (!is_ipcomp || pfs_group != NULL)
    {
        ...
        {
            loglog(RC_LOG_SERIOUS, "GROUP_DESCRIPTION inconsistent with that of %s in IPsec SA", selection ? "the Proposal" : "a previous Transform");
            return FALSE;
        }
    }
              ...
              bool tunnel_mode)                  /* true for inner most tunnel SA */
{
    ...
    if (r_proposal.isap_protoid == PROTO_IPCOMP)
    {
        ...
        out_raw((u_char *)&pi->our_spi + IPSEC_DOI_SPI_SIZE - IPCOMP_CPI_SIZE, IPCOMP_CPI_SIZE, &r_proposal_pbs, "CPI");
    }
    else
    {
        get_ipsec_spi(pi, r_proposal.isap_protoid == PROTO_IPSEC_AH ? IPPROTO_AH : IPPROTO_ESP, st, tunnel_mode);

        /* XXX should check for errors */
        out_raw((u_char *)&pi->our_spi, IPSEC_DOI_SPI_SIZE, &r_proposal_pbs, "SPI");
    }
    ...
    if (!out_raw(trans_pbs->cur, pbs_left(trans_pbs), &r_trans_pbs, "attributes"))
        impossible();
    ...
    while (next_full)
    {
        ...
        do
        {
            ...
            if (next_proposal.isap_protoid == PROTO_IPCOMP)
            {
                ...
                if (next_proposal.isap_spisize == IPSEC_DOI_SPI_SIZE)
                {
                    ...
                    if (!in_raw(filler, sizeof(filler), &next_proposal_pbs, "CPI filler") || !all_zero(filler, sizeof(filler)))
                        return INVALID_SPI;
                }
                else if (next_proposal.isap_spisize != IPCOMP_CPI_SIZE)
                {
                    loglog(RC_LOG_SERIOUS, "IPsec Proposal with improper CPI size (%u)", next_proposal.isap_spisize);
                    return INVALID_SPI;
                }
                 ...
                 */
                zero(&next_spi);
                if (!in_raw((u_char *)&next_spi + IPSEC_DOI_SPI_SIZE - IPCOMP_CPI_SIZE, IPCOMP_CPI_SIZE, &next_proposal_pbs, "CPI"))
                    return INVALID_SPI;
                ...
                case IPCOMP_DEFLATE:
                    ...
                    if (next_spi == 0)
                    {
                        loglog(RC_LOG_SERIOUS, "IPsec Proposal contains well-known CPI that I cannot uniquify");
                        return INVALID_SPI;
                    }
                    break;
                default:
                    if (ntohl(next_spi) < IPCOMP_FIRST_NEGOTIATED || ntohl(next_spi) > IPCOMP_LAST_NEGOTIATED)
                    {
                        loglog(RC_LOG_SERIOUS, "IPsec Proposal contains CPI from non-negotiated range (0x%lx)", (unsigned long)ntohl(next_spi));
                        return INVALID_SPI;
                    }
                    break;
                }
            ...
            else
            {
                ...
                if (next_proposal.isap_spisize != IPSEC_DOI_SPI_SIZE)
                {
                    loglog(RC_LOG_SERIOUS, "IPsec Proposal with improper SPI size (%u)", next_proposal.isap_spisize);
                    return INVALID_SPI;
                }
                ...
                {
                    loglog(RC_LOG_SERIOUS, "IPsec Proposal contains invalid SPI (0x%lx)", (unsigned long)ntohl(next_spi));
                    return INVALID_SPI;
                }
            }
            ...
            default:
                loglog(RC_LOG_SERIOUS, "unexpected Protocol ID (%s) in IPsec Proposal", enum_show(&protocol_names, next_proposal.isap_protoid));
                return INVALID_PROTOCOL_ID;
            }
            ...
            {
                loglog(RC_LOG_SERIOUS, "unexpected in Proposal: %s", enum_show(&payload_names, next_proposal.isap_np));
                return BAD_PROPOSAL_SYNTAX;
            }
        ...
        if (ah_seen)
        {
            ...
            for (tn = 0; tn != ah_proposal.isap_notrans; tn++)
            {
                ...
                if (!parse_ipsec_transform(&ah_trans, &ah_attrs, &ah_prop_pbs, &ah_trans_pbs, &isakmp_ah_transform_desc, previous_transnum, selection, tn == ah_proposal.isap_notrans - 1, FALSE, st))
                    return BAD_PROPOSAL_SYNTAX;
                ...
                {
                    loglog(RC_LOG_SERIOUS, "%s attribute inappropriate in %s Transform", enum_name(&auth_alg_names, ah_attrs.transattrs.integ_hash), enum_show(&ah_transformid_names, ah_attrs.transattrs.encrypt));
                    return BAD_PROPOSAL_SYNTAX;
                }
                if (!ok_auth)
                {
                    DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("%s attribute unsupported"
                                                         " in %s Transform from %s",
                                                         enum_name(&auth_alg_names, ah_attrs.transattrs.integ_hash), enum_show(&ah_transformid_names, ah_attrs.transattrs.encrypt), ip_str(&c->spd.that.host_addr)));
        ...
        if (esp_seen)
        {
            ...
            {
                if (!parse_ipsec_transform(&esp_trans, &esp_attrs, &esp_prop_pbs, &esp_trans_pbs, &isakmp_esp_transform_desc, previous_transnum, selection, tn == esp_proposal.isap_notrans - 1, FALSE, st))
                    return BAD_PROPOSAL_SYNTAX;

                previous_transnum = esp_trans.isat_transnum;

                ugh = kernel_alg_esp_enc_ok(esp_attrs.transattrs.encrypt, esp_attrs.transattrs.enckeylen, c->alg_info_esp);

                if (ugh != NULL)
                {
                    loglog(RC_LOG_SERIOUS, "proposal refused due to: %s", ugh);
                    loglog(RC_LOG_SERIOUS, "unsupported ESP Transform %s from %s", enum_show(&esp_transformid_names, esp_attrs.transattrs.encrypt), ip_str(&c->spd.that.host_addr));
                    continue; /* try another */
                }

                if (!ESP_AALG_PRESENT(esp_attrs.transattrs.integ_hash))
                {
                    loglog(RC_LOG_SERIOUS, "unsupported ESP auth alg %s from %s", enum_show(&trans_type_integ_names, esp_attrs.transattrs.integ_hash), ip_str(&c->spd.that.host_addr));
                    continue; /* try another */
                }
        ...
        {
            DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("policy for \"%s\" requires encryption but ESP not in Proposal from %s", c->name, ip_str(&c->spd.that.host_addr)));
            continue; /* we needed encryption, but didn't find ESP */
        }
        else if ((st->st_policy & POLICY_AUTHENTICATE) && !ah_seen)
        {
            DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("policy for \"%s\" requires authentication"
        ...
        if (ipcomp_seen)
        {
            ...
            {
                plog("compression proposed by %s, but policy for \"%s\" forbids it", ip_str(&c->spd.that.host_addr), c->name);
                continue; /* unwanted compression proposal */
            }
#endif
            if (!can_do_IPcomp)
            {
                openswan_log("compression proposed by %s, but KLIPS is not configured with IPCOMP", ip_str(&c->spd.that.host_addr));
                continue;
            }
            ...
            {
                if (!parse_ipsec_transform(&ipcomp_trans, &ipcomp_attrs, &ipcomp_prop_pbs, &ipcomp_trans_pbs, &isakmp_ipcomp_transform_desc, previous_transnum, selection, tn == ipcomp_proposal.isap_notrans - 1, TRUE, st))
                    return BAD_PROPOSAL_SYNTAX;
                ...
                default:
                    DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("unsupported IPCOMP Transform %s from %s", enum_show(&ipcomp_transformid_names, ipcomp_attrs.transattrs.encrypt), ip_str(&c->spd.that.host_addr)));
                    continue; /* try another */
                }
                ...
                if (ah_seen && ah_attrs.encapsulation != ipcomp_attrs.encapsulation)
                {
                    ...
                    DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("AH and IPCOMP transforms disagree about encapsulation; TUNNEL presumed"));
                }
                else if (esp_seen && esp_attrs.encapsulation != ipcomp_attrs.encapsulation)
                {
                    /* ??? This should be an error, but is it? */
                    DBG(DBG_CONTROL | DBG_CRYPT, DBG_log("ESP and IPCOMP transforms disagree about encapsulation; TUNNEL presumed"));
                }
        ...
        if (r_sa_pbs != NULL)
        {
            ...
            /* AH proposal */
            if (ah_seen)
                echo_proposal(st, ah_proposal, ah_trans, esp_seen || ipcomp_seen ? ISAKMP_NEXT_P : ISAKMP_NEXT_NONE, r_sa_pbs, &st->st_ah, &isakmp_ah_transform_desc, &ah_trans_pbs, tunnel_mode && inner_proto == IPPROTO_AH);

            /* ESP proposal */
            if (esp_seen)
                echo_proposal(st, esp_proposal, esp_trans, ipcomp_seen ? ISAKMP_NEXT_P : ISAKMP_NEXT_NONE, r_sa_pbs, &st->st_esp, &isakmp_esp_transform_desc, &esp_trans_pbs, tunnel_mode && inner_proto == IPPROTO_ESP);

            /* IPCOMP proposal */
            if (ipcomp_seen)
                echo_proposal(st, ipcomp_proposal, ipcomp_trans, ISAKMP_NEXT_NONE, r_sa_pbs, &st->st_ipcomp, &isakmp_ipcomp_transform_desc, &ipcomp_trans_pbs, tunnel_mode && inner_proto == IPPROTO_COMP);
        }
        if (r_sa_pbs != NULL)
            close_output_pbs(r_sa_pbs);
