...
#define elemsof(array) (sizeof(array) / sizeof(*(array))) /* number of elements in an array  数组中的元素数 */
...
extern const char *bitnamesofb(const char *const table[], lset_t val, char *buf, size_t blen);
...
extern const char *keyword_name(const struct keyword_enum_values *kevs, unsigned int value, char namebuf[KEYWORD_NAME_BUFLEN]);
