...
{
    //密钥ID
    char keyid[KEYID_BUF]; /* see ipsec_keyb†lobtoid(3) 参见 ipsec_keyb†lobtoid(3)*/
    /*
     * length of modul†us n in octets: [RSA_MIN_OCTETS, RSA_MAX_OCTETS]
     * 以八位字节为单位的模数 n 的长度：[RSA_MIN_OCTETS, RSA_MAX_OCTETS]
     */
    //长度
    unsigned k;

    chunk_t key_rfc3110; /* Raw Public key format   原始公钥格式*/
...
extern void delete_public_keys(struct pubkey_list **head, const struct id *id, enum pubkey_alg alg);
...
extern err_t add_public_key(const struct id *id, enum dns_auth_level dns_auth_level, enum pubkey_alg alg, const chunk_t *key, struct pubkey_list **head);

extern bool same_RSA_public_key(const struct RSA_public_key *a, const struct RSA_public_key *b);

extern void install_public_key(osw_public_key *pk, struct pubkey_list **head);

extern void free_public_key(struct pubkey *pk);

extern void osw_load_preshared_secrets(struct secret **psecrets, int verbose, const char *secrets_file, prompt_pass_t *pass, const char *root_dir);
...
extern struct secret *osw_find_secret_by_id(struct secret *secrets, enum PrivateKeyKind kind, const struct id *my_id, osw_public_key *key1, osw_public_key *key2, const struct id *his_id, bool asym);
