...
typedef void alg_info_adder(struct alg_info *alg_info, enum ikev2_trans_type_encr ealg_id, int ek_bits, enum ikev2_trans_type_integ aalg_id, int ak_bits, enum ikev2_trans_type_prf prfalg_id UNUSED, enum ikev2_trans_type_dh modp_id);
...
struct alg_info_esp *alg_info_esp_create_from_str(const char *alg_str, err_t *err_p);

struct alg_info_esp *alg_info_ah_create_from_str(const char *alg_str, err_t *err_p);

struct alg_info_ike *alg_info_ike_create_from_str(const char *alg_str, err_t *err_p);
...
extern char *alg_info_snprint_ike2(struct ike_info *ike_info, int eklen, int aklen, int *usedsize, char *buf, int buflen);
...
extern int alg_enum_search_ppfix(enum_names *ed, const char *prefix, const char *postfix, const char *str, int str_len);

struct parser_context;
struct oakley_group_desc;
extern int alg_info_parse_str(struct alg_info *alg_info, const char *alg_str, const char **err_p, void (*parser_init)(struct parser_context *p_ctx), alg_info_adder alg_info_add, const struct oakley_group_desc *(*lookup_group)(enum ikev2_trans_type_dh group));
