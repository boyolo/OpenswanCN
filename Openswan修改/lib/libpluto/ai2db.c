...
struct db_sa *alginfo2parent_db2(struct alg_info_ike *ai)
{
    ...
    ALG_INFO_IKE_FOREACH((struct alg_info_ike *)ai, ike_info, cnt)
    {
        ...
        alg_info_snprint_ike2(ike_info, ike_info->ike_eklen, ike_info->ike_hklen, &ret, ptr, sizeof(missing_alg_buf));
        ...
        if (!enc_present || !integ_present || !prf_present || !group_present)
        {

            DBG(DBG_EMITTING,
                DBG_log("not including %s in policy, as algorithm missing"
                        "(enc:%u,integ:%u,prf:%u,group:%u)",
                        missing_alg_buf, enc_present, integ_present, prf_present, group_present));
            continue;
        }
