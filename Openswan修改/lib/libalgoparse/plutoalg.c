                   ...
                   unsigned int modp_id)
{
    ...
    for (i = 0; i < cnt; i++)
    {
        if (ike_info[i].ike_ealg == ealg_id && (!ek_bits || ike_info[i].ike_eklen == ek_bits) && ike_info[i].ike_halg == aalg_id && (!ak_bits || ike_info[i].ike_hklen == ak_bits) && ike_info[i].ike_modp == modp_id)
        {
            return;
        }
    }
 ...
 */
static void
alg_info_ike_add(struct alg_info *alg_info, enum ikev2_trans_type_encr ealg_id, int ek_bits, enum ikev2_trans_type_integ aalg_id, int ak_bits, enum ikev2_trans_type_prf prfalg_id, enum ikev2_trans_type_dh modp_id)
{
...
alg_info_snprint_esp(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
    ...
    ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt)
    {
        ...
        ret = snprintf(ptr, buflen, "%s%s(%d)_%03d-%s(%d)_%03d", sep, enum_name(&trans_type_encr_names, esp_info->esp_ealg_id), esp_info->esp_ealg_id, eklen, enum_name(&trans_type_integ_names, esp_info->esp_aalg_id), esp_info->esp_aalg_id, aklen);
...
alg_info_snprint_ah(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
    ...
    ALG_INFO_ESP_FOREACH(alg_info, esp_info, cnt)
    {
        ...
        ret = snprintf(ptr, buflen, "%s%s(%d)_%03d", sep, enum_name(&auth_alg_names, esp_info->esp_aalg_id) + sizeof("AUTH_ALGORITHM_HMAC"), esp_info->esp_aalg_id, aklen);
...
void alg_info_snprint_phase2(char *buf, size_t buflen, struct alg_info_esp *alg_info)
{
...
char *alg_info_snprint_ike2(struct ike_info *ike_info, int eklen, int aklen, int *usedsize, char *buf, int buflen)
{
    int ret;
    char *curbuf = buf;
    ...
    if (eklen == 0)
    {
        ret = snprintf(curbuf, buflen - 1, "%s(%d)", encname, ike_info->ike_ealg);
    }
    else
    {
        ret = snprintf(curbuf, buflen - 1, "%s(%d)_%03d", encname, ike_info->ike_ealg, eklen);
    }
    if (ret <= 0)
    ...
    if (aklen == 0)
    {
        ret = snprintf(curbuf, buflen - 1, "-%s(%d)", hashname, ike_info->ike_halg);
    }
    else
    {
        ret = snprintf(curbuf, buflen - 1, "-%s(%d)_%03d", hashname, ike_info->ike_halg, aklen);
    }
    if (ret <= 0)
        return "invalid ikeinfo";
    curbuf += ret;
    ...
    ret = snprintf(curbuf, buflen - 1, "-%s(%d)-%s(%d)", prfname, ike_info->ike_prfalg, modpname, ike_info->ike_modp);
    buflen -= ret;
...
void alg_info_snprint_ike(char *buf, size_t buflen, struct alg_info_ike *alg_info)
{
    ...
    /*尝试*/
    /*输出调试*/
    //#define F_PATH "/home/parallels/Desktop/111.txt"
    //    FILE *stream;
    //    stream = fopen(F_PATH, "at+");
    //    fprintf(stream, "%d\n", sizeof(int));
    //    fclose(stream);
    //    stream = NULL;
    // key_t key;
    ...
    // if ((ike_info = (struct ike_info *)shmat(shmid, 0, 0)) == (struct ike_info *)-1)
    //{
    //     perror("shmat");
    //     exit(1);
    // }
    ...
    struct ike_info *ike_info;
    ike_info = (struct ike_info *)getIkeInfoAddr(0, 0);
    int cnt;
    int eklen, aklen;
    const char *sep = "";
    ...
    {
        if (ike_alg_enc_present(ike_info->ike_ealg, ike_info->ike_eklen) && (ikev2_alg_integ_present(ike_info->ike_halg, ike_info->ike_hklen)) && (ike_alg_prf_present(ike_info->ike_prfalg)) && (lookup_group(ike_info->ike_modp)))
        {
 ...
 */
bool kernel_alg_esp_ok_final(int ealg, unsigned int key_len, int aalg, struct alg_info_esp *alg_info)
{
    int ealg_insecure;
    /*
...
#ifndef USE_1DES
                    if (ealg_insecure)
                        loglog(RC_LOG_SERIOUS, "You should NOT use insecure ESP algorithms [%s (%d)]!", enum_name(&esp_transformid_names, ealg), key_len);
#endif
                    return TRUE;
                }
            }
