 ...
 */
int alg_enum_search_prefix(enum_names *ed, const char *prefix, const char *str, int str_len)
{
	char buf[64];
	char *ptr;
	int ret;
...
static int
parser_alg_info_add(struct parser_context *p_ctx, struct alg_info *alg_info, alg_info_adder *alg_info_add, const struct oakley_group_desc *(*lookup_group)(enum ikev2_trans_type_dh group))
{
	...
	if (p_ctx->ealg_permit && *p_ctx->ealg_buf)
	{
		...
		if (ealg_id == IKEv2_ENCR_AES_GCM_8 || ealg_id == IKEv2_ENCR_AES_GCM_12 || ealg_id == IKEv2_ENCR_AES_GCM_16)
		{

			/* AES-GCM length key length + 4 bytes (32 bits) */
			if (p_ctx->eklen != 128 && p_ctx->eklen != 192 && p_ctx->eklen != 256)
			{
	...
	(*alg_info_add)(alg_info, ealg_id, p_ctx->eklen, aalg_id, p_ctx->aklen, prfalg_id, modp_id);
	return 0;
out:
	return -1;
}

int alg_info_parse_str(struct alg_info *alg_info, const char *alg_str, const char **err_p, void (*parser_init)(struct parser_context *p_ctx), alg_info_adder *alg_info_add, const struct oakley_group_desc *(*lookup_group)(enum ikev2_trans_type_dh group))
{
	...
	*err_buf = 0;
	...
	for (ret = 0, ptr = alg_str; ret < ST_EOF;)
	{
		...
		case ST_EOF:
			...
			if (parser_alg_info_add(&ctx, alg_info, alg_info_add, lookup_group) < 0)
			{
...
alg_info_esp_create_from_str(const char *alg_str, const char **err_p)
{
	...
	ret = alg_info_parse_str((struct alg_info *)alg_info_esp, alg_str, err_p, parser_init_esp, alg_info_esp_add, NULL);
...
struct alg_info_esp *
alg_info_ah_create_from_str(const char *alg_str, const char **err_p)
{
	struct alg_info_esp *alg_info_esp;
	int ret = 0;
	...
	ret = alg_info_parse_str((struct alg_info *)alg_info_esp, alg_str, err_p, parser_init_ah, alg_info_ah_add, NULL);
...
int alg_info_snprint(char *buf, int buflen, struct alg_info *alg_info)
{
	...
	case PROTO_IPSEC_ESP:
	{
		...
		{
			snprintf(ptr, buflen, "%s(%d)_%03d-%s(%d)_%03d", enum_name(&trans_type_encr_names, esp_info->esp_ealg_id), esp_info->esp_ealg_id, (int)esp_info->esp_ealg_keylen, enum_name(&trans_type_integ_names, esp_info->esp_aalg_id), esp_info->esp_aalg_id, (int)esp_info->esp_aalg_keylen);
			...
			{
				snprintf(ptr, buflen, "-%s(%d)", alg_info_modp_shortname(esp_info->pfs_group), esp_info->pfs_group);
				size_t np = strlen(ptr);
				ptr += np;
				buflen -= np;
				if (buflen <= 0)
	...
	case PROTO_IPSEC_AH:
	{
		...
		ALG_INFO_ESP_FOREACH(alg_info_esp, esp_info, cnt)
		{
			snprintf(ptr, buflen, "%s(%d)_%03d", enum_name(&trans_type_integ_names, esp_info->esp_aalg_id), esp_info->esp_aalg_id, (int)esp_info->esp_aalg_keylen);
			size_t np = strlen(ptr);
			ptr += np;
			buflen -= np;
			if (esp_info->pfs_group != OAKLEY_INVALID_GROUP)
	...
	if (buflen > 0)
	{
		snprintf(ptr, buflen, "; flags=%s",
						 alg_info->alg_info_flags & ALG_INFO_F_STRICT ? "strict" : "-strict");
		size_t np = strlen(ptr);
		ptr += np;
		buflen -= np;
	}
