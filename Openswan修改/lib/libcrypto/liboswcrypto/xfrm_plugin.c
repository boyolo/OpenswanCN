 ...
 */
bool ikev1_alg_ok_final(int ealg, unsigned key_len, int aalg, unsigned int group, struct alg_info_ike *alg_info_ike)
{
	/*
			...
			(alg_info_ike && alg_info_ike->alg_info_flags & ALG_INFO_F_STRICT))
	{
		...
		if (alg_info_ike)
		{
			ALG_INFO_IKE_FOREACH(alg_info_ike, ike_info, i)
			{
				...
				{
					if (ealg_insecure)
						loglog(RC_LOG_SERIOUS, "You should NOT use insecure IKE algorithms (%s)!", enum_name(&oakley_enc_names, ealg));
					return TRUE;
				}
			}
		}
 ...
 */
struct ike_alg *
ike_alg_ikev1_find(enum ikev2_trans_type algo_type, unsigned algo_id, unsigned keysize UNUSED)
{
 ...
 */
struct ike_alg *
ike_alg_ikev2_find(enum ikev2_trans_type algo_type, enum ikev2_trans_type_encr algo_v2id, unsigned keysize __attribute__((unused)))
{
	struct ike_alg *e = ike_alg_base[algo_type];
	for (; e != NULL; e = e->algo_next)
	{
