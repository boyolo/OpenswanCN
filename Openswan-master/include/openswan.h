 ...
 * Yes Virginia, we have started a windows port.
 是的，Virginia，我们已经启动了 Windows 端口。
 */
...
/*
 * Basic data types for the address-handling functions.  地址处理功能的基本数据类型。
 * ip_address and ip_subnet are supposed to be opaque types; do not use their definitions directly, they are subject to change!
 * IP_Address和IP_SUBNET应该是不透明类型; 不要直接使用他们的定义，他们可能会有所变化！
 */
...
struct sockaddr_in6
{
	...
	__u16 sin6_port;								/* Transport layer port # * 运输层端口 */
...
#define ip_address_cmp(a, b)                           \
	(ip_address_family((a)) != ip_address_family((b)) || \
	 (ip_address_family((a)) == AF_INET ? ((a)->u.v4.sin_addr.s_addr != (b)->u.v4.sin_addr.s_addr) : memcmp((a)->u.v6.sin6_addr.s6_addr32, (b)->u.v6.sin6_addr.s6_addr32, sizeof(u_int32_t) * 4)))
#define ip_address_isany(a)                                                     \
	(ip_address_family((a)) == AF_INET6 ? ((a)->u.v6.sin6_addr.s6_addr[0] == 0 && \
...
size_t datatot(const unsigned char *src, size_t srclen, int format, char *buf, size_t buflen);
...
#define KEYID_BUF 10 /* up to 9 text digits plus NUL 最多 9 个文本数字加上 NUL */
