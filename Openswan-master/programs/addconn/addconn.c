...
int main(int argc, char *argv[])
{
	...
	if (all)
	{
		...
		{
			if (conn->desired_state == STARTUP_ADD || conn->desired_state == STARTUP_START || conn->desired_state == STARTUP_ROUTE)
			{
				if (verbose)
					printf(" %s", conn->name);
	...
	else if (listroute)
	{
		...
		for (conn = cfg->conns.tqh_first;
				 ...
				 conn = conn->link.tqe_next)
		{
			if (conn->desired_state == STARTUP_START || conn->desired_state == STARTUP_ROUTE)
			{
				printf("%s ", conn->name);
			}
		}
		...
		for (connum = optind; connum < argc; connum++)
		{
			...
			if (conn == NULL)
			{
				...
				for (conn = cfg->conns.tqh_first;
						 ...
						 conn = conn->link.tqe_next)
				{
					if (conn->strings_set[KSF_CONNALIAS] && osw_alias_cmp(connname, conn->strings[KSF_CONNALIAS]))
					{
